...
async def write_poc(cve_id: str, poc_object: CveModelForPoC, path: str):
    where_to_write = os.path.join(path, f"{cve_id}.md")
    ready_poc_object = await _return_ready_poc_object(cve_id,poc_object)
...
async def _return_ready_poc_object(cve_id: str, cve_model: CveModelForPoC) -> CvePoc:
